...
- One central Chopperbot personality shared across every server and DM
...
- Per-user trait profiles (curiosity, humor, logic, ...) evolve from conversations and shape responses via archetypes
...
- Automatic web search when a message needs fresh information (time-sensitive questions, current events) — with timeouts, retries, and graceful fallback when the search endpoint is down
...
**personalities.py** — Central personality

- Single `chopperbot` instance used everywhere via `get_personality()`
...
| `user_personality` | Per-user evolving trait profiles |
